...
const PayRate: React.FC = () => {
  ...
  const addNewRate = () => {
    ...
    setPayRates([...payRates, newRate]);
  };

  const removeRate = (id: string) => {
    setPayRates(payRates.filter(rate => rate.id !== id));
  };

  const updateRate = (id: string, field: keyof PayRateRow, value: string | boolean) => {
    setPayRates(payRates.map(rate => 
      rate.id === id ? { ...rate, [field]: value } : rate
    ));
  };

  const handleSave = () => {
    console.log('Saving pay rates:', payRates);
    // Add save logic here
  };
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Tooltip title="Pay rate information">
